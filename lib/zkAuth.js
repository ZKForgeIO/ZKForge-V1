...
// ---------- Ed25519 Public Key Validation (Issue #5) ----------

/**
 * Validate that a base58-encoded Ed25519 public key is exactly 32 bytes.
 * @param {string} publicKeyBase58 - Base58-encoded public key
 * @returns {boolean} - True if valid 32-byte Ed25519 public key
 */
export function validateEd25519PublicKey(publicKeyBase58) {
...
// ---------- Nonce-Based Challenge (Issue #2/#4) ----------

const CHALLENGE_TTL_MS = 5 * 60 * 1000; // 5 minutes

/**
 * Generate a fresh authentication challenge with nonce and timestamp.
 * @returns {{ nonce: string, timestamp: number, expiresAt: number }}
 */
export function generateAuthChallenge() {
  const nonce = bs58.encode(nacl.randomBytes(32));
  const timestamp = Date.now();
  const expiresAt = timestamp + CHALLENGE_TTL_MS;
  return { nonce, timestamp, expiresAt };
}

/**
 * Verify that a challenge nonce is valid (not expired, matches stored).
 * @param {string} proofNonce - Nonce from the proof
 * @param {string} storedNonce - Nonce stored on the server
 * @param {Date|number} storedTime - When the challenge was created
 * @returns {{ valid: boolean, reason?: string }}
 */
export function validateChallenge(proofNonce, storedNonce, storedTime) {
  if (!proofNonce || !storedNonce) {
  ...
  const challengeTime = typeof storedTime === 'number' ? storedTime : new Date(storedTime).getTime();
...
/**
 * Verify a zkSTARK-style auth proof with nonce challenge validation.
 * Adds replay protection by requiring a fresh, server-issued challenge.
 *
 * @param {string} finalHashHex - The 0x-hex commitment stored in Profile.zk_public_key
 * @param {object} rawProof - JSON proof object with required `challenge` field
 * @param {object} paramsOverride - Optional { steps, queries }
 * @param {object} challengeData - { storedNonce, storedTime } from Profile
 * @returns {{ valid: boolean, reason?: string }}
 */
export function verifyStarkAuthProofWithChallenge(finalHashHex, rawProof, paramsOverride = {}, challengeData = null) {
  // 1. Challenge validation is ALWAYS required when challengeData is provided
  // This prevents replay attacks by ensuring each proof uses a fresh, one-time nonce
  if (challengeData) {
    const { storedNonce, storedTime } = challengeData;
    const proofNonce = rawProof?.challenge;

    // If there's no stored nonce (challenge already used or never generated), reject
    if (!storedNonce) {
      return { valid: false, reason: 'no active challenge - please request a new one' };
    }

    const challengeResult = validateChallenge(proofNonce, storedNonce, storedTime);
    if (!challengeResult.valid) {
      return { valid: false, reason: challengeResult.reason };
    }
  }

  // 2. Verify the cryptographic proof
  try {
    const isValid = verifyStarkAuthProof(finalHashHex, rawProof, paramsOverride);
    return { valid: isValid, reason: isValid ? undefined : 'proof verification failed' };
  } catch (e) {
    return { valid: false, reason: e.message || 'proof verification error' };
  }
}
